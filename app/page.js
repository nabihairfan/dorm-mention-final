...
export default function ConfessionsBoard() {
  ...
  const handleVote = async (captionId, voteValue) => {
    ...
    // 2. UPDATE BOTTOM SCOREBOARD IMMEDIATELY
    setMyStats(prev => {
      let newFireTotal = prev.fire;
      let newTrashTotal = prev.trash;

      if (isRemoving) {
        voteValue === 1 ? newFireTotal-- : newTrashTotal--;
      } else {
        // If switching from Fire to Trash
        if (currentCard?.userVote === 1) { newFireTotal--; newTrashTotal++; }
        // If switching from Trash to Fire
        else if (currentCard?.userVote === -1) { newTrashTotal--; newFireTotal++; }
        // If it's a brand new vote
        else { voteValue === 1 ? newFireTotal++ : newTrashTotal++; }
      }
      return { fire: Math.max(0, newFireTotal), trash: Math.max(0, newTrashTotal) };
    });
  ...
  return (
    <div style={styles.page}>
      ...
      <div style={styles.controlCenter}>
        ...
        <button onClick={resetAllVotes} style={styles.resetBtn}>RESET ALL VOTES</button>
      </div>
...
const styles = {
  ...
  scoreItem: { fontSize: '24px', fontWeight: '900', fontFamily: "'Luckiest Guy', cursive" },
  ...
  logo: { fontSize: '32px', color: '#000', fontFamily: "'Luckiest Guy', cursive" },
  ...
  loader: { height: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '30px', fontWeight: '900', fontFamily: "'Luckiest Guy', cursive" }
};